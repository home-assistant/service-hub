...
export class ComponentsCommand implements DiscordTransformedCommand<ComponentsDto> {
  ...
  async handler(
    ...
    if (component === 'reload') {
      await this.componentDataService.ensureData(channel, true);
    ...
    const componentData = await this.componentDataService.getComponent(channel, component);
  ...
  async onInteractionCreate(interaction: AutocompleteInteraction): Promise<void> {
    ...
    try {
      await this.componentDataService.ensureData(channel);
      ...
      await interaction.respond(
        focusedValue.length !== 0
          ? Object.entries(this.componentDataService.data[channel])
              .map(([component, data]) => ({
                name: data.title,
                value: component,
              }))
