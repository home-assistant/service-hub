...
import { prLabelChangeType } from "../../src/rules-pr/pr-label-change-type.js";
...
describe("pr-label-change-type", () => {
  describe("type of change from body", () => {
    it("adds bugfix label from a single checked task", async () => {
      ...
      const result = await runRule(prLabelChangeType, context);
  ...
  it("is excluded from bot senders by the dispatcher", () => {
    expect(prLabelChangeType.allowBots).toBe(false);
  });
