...
import { prMergeTarget } from "../../src/rules-pr/pr-merge-target.js";
...
describe("pr-merge-target", () => {
  it("passes when the PR targets `dev`", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("dev"));
  ...
  it("fails when a non-member targets `master`", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("master", "NONE"));
  ...
  it("fails when a non-member targets `rc`", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("rc", "CONTRIBUTOR"));
    expect(result?.dashboard?.status).toBe("fail");
    expect(result?.dashboard?.message).toContain("`rc`");
  });

  it("fails when a non-member targets an arbitrary branch", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("some-feature-branch"));
    expect(result?.dashboard?.status).toBe("fail");
  ...
  it("downgrades to info when an org MEMBER targets a non-dev branch", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("master", "MEMBER"));
  ...
  it("downgrades to info for OWNER and COLLABORATOR too", async () => {
    const owner = await runRule(prMergeTarget, contextTargeting("rc", "OWNER"));
    expect(owner?.dashboard?.status).toBe("info");

    const collab = await runRule(prMergeTarget, contextTargeting("rc", "COLLABORATOR"));
    expect(collab?.dashboard?.status).toBe("info");
  });

  it("does not emit any labels", async () => {
    const result = await runRule(prMergeTarget, contextTargeting("master"));
