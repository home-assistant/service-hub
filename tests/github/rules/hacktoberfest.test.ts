...
import { hacktoberfest } from "../../../src/github/rules/hacktoberfest.js";
...
describe("hacktoberfest", () => {
  ...
  describe("PR opened", () => {
    it("adds Hacktoberfest label in October when repo has topic", async () => {
      ...
      const result = await runRule(hacktoberfest, context);
  ...
  it("listens to opened and closed events", () => {
    expect(Object.keys(hacktoberfest.events)).toContain(EventType.PULL_REQUEST_OPENED);
    expect(Object.keys(hacktoberfest.events)).toContain(EventType.PULL_REQUEST_CLOSED);
  });
});
